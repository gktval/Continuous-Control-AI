...
class Agent():
    ...
    def __init__(self, state_size, action_size, config):
        ...
        self.critic_local=model.Critic(state_size,action_size).to(self.device)
        self.critic_target=model.Critic(state_size,action_size).to(self.device)
    ...
    def learn(self, experiences):
        ...
        pred = self.critic_target.forward(torch.cat((next_states, next_actions), dim=1)).detach()     
        ...
        if self.priority_replay:
            ...
            critic_loss = (td_error.pow(2)*weights).mean().to(self.device)       
            self.memory.updatePriorities(idx, abs(td_error.data.cpu().numpy()))
        else:
        ...
        actor_loss = -self.critic_local.forward(torch.cat((states, actions_actor), dim=1)).mean()  

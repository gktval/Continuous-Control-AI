...
class Agent():
    ...
    def __init__(self, state_size, action_size, config):
        ...
        self.critic_local=model.CriticDist(state_size,action_size, self.v_min, self.v_max, self.num_atoms).to(self.device)
        self.critic_target=model.CriticDist(state_size,action_size, self.v_min, self.v_max, self.num_atoms).to(self.device)
        self.critic_optim=optim.Adam(self.critic_local.parameters(), lr=self.lr_critic) #, weight_decay=self.weight_decay )

        self.value_criterion = torch.nn.BCELoss(reduction='none')
        ...
        # Noise process
        if self.is_noisy:
            self.noise = noise.OUNoise2(action_size)
    ...
    def act(self, states):
        ...
        if self.is_noisy:
             action += self.noise.get_action(self.step_count)
    ...
    def learn(self, experiences):
        states, actions, rewards, next_states, dones, inds, weights = experiences

         # ------------------- update critic ------------------- #       
        next_actions = self.actor_target(next_states)
        target_pred = self.critic_target.get_probs(next_states, next_actions) 
        ...
        Q_expected = self.critic_local.get_probs(states, actions)
        
        prob_dist = self.value_criterion(Q_expected, proj_distr)
        critic_loss = prob_dist.mean(axis=1)

        # Update priorities in buffer
        td_error = critic_loss.cpu().detach().numpy().flatten()
        if self.priority_replay:
            weights_update = np.abs(td_error) + self.priority_eps
            self.memory.updatePriorities(inds, weights_update)
        ...
        actor_loss = self.critic_local.get_probs(states, actions_actor)

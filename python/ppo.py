...
def Run(env, config, n_episodes=200, max_t=1000):   
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for _ in range(max_t):
            ...
            agent.step(states, actions, rewards, next_states, dones, log_probs, values)       # step through agent learning 
        ...
        agent.set_learning_rate(agent.lr_actor)
...
def saveCheckpoints(filename, agent, config):
    ...
    torch.save(agent.network.state_dict(), config.model_path + filename + '_checkpoint-actor.pth')
...
def watchAgent(config, env):
    ...
    agent = Agent(state_size=state_size, action_size=action_size, config=config)
    agent.network.load_state_dict(torch.load("checkpoints/ppo_checkpoint-actor.pth"))
    
    for j in range(10000):
        action, log_probs, values = agent.act(states)

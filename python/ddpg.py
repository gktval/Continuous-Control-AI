...
def watchAgent(config, env):
    ...
    agent = Agent(state_size=state_size, action_size=action_size, config=config)
    agent.actor_local.load_state_dict(torch.load("checkpoints/ddpg_checkpoint-actor.pth"))
    agent.critic_local.load_state_dict(torch.load("checkpoints/ddpg_checkpoint-critic.pth"))
